...
WITH AggregatedImpressions AS (
  ...
  INNER JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableAdvertisers`
    AS Advertisers
    ON
      Advertisers.account_id = Impressions.account_id
      AND Advertisers.advertiser_id = Impressions.advertiser_id
  ...
  LEFT JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableBrowsers`
    AS Browsers
    ON
      Browsers.account_id = Impressions.account_id
      AND Browsers.browser_platform_id = Impressions.browser_platform_id
  LEFT JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableStates`
    AS States
    ON
      States.account_id = Impressions.account_id
