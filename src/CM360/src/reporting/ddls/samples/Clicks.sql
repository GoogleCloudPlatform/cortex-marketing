...
WITH AggregatedClicks AS (
  ...
  INNER JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.Impressions`
    AS Impressions
  ...
  INNER JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableAdvertisers`
    AS Advertisers
    ON
      Advertisers.account_id = Clicks.account_id
      AND Advertisers.advertiser_id = Clicks.advertiser_id
  ...
  LEFT JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableBrowsers`
    AS Browsers
    ON
      Browsers.account_id = Clicks.account_id
      AND Browsers.browser_platform_id = Clicks.browser_platform_id
  LEFT JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableStates`
    AS States
    ON
      States.account_id = Clicks.account_id
