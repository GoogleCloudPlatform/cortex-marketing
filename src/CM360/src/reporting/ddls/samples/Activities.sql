...
WITH AggregatedActivities AS (
  ...
  INNER JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableAdvertisers`
    AS Advertisers
    ON
      Advertisers.account_id = Activities.account_id
      AND Advertisers.advertiser_id = Activities.advertiser_id
  ...
  LEFT JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableBrowsers`
    AS Browsers
    ON
      Browsers.account_id = Activities.account_id
      AND Browsers.browser_platform_id = Activities.browser_platform_id
  LEFT JOIN `{{ project_id_tgt }}.{{ marketing_cm360_datasets_reporting }}.MatchTableStates`
    AS States
    ON
      States.account_id = Activities.account_id
